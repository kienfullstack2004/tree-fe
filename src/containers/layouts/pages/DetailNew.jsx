...
import { apiGetAllCommentMedicine, apiGetAllCountComment, apiGetAllPost } from "../../../Service/apiPost";
...
const DetailNew = () => {
  ...
  useEffect(() => {
    const fectchPost = async () => {
      ...
      const responsive2 = await apiGetAllCommentMedicine();
      ...
      path === "medicine" && setMedicineOne(res);
      path === "news" && setPostDetail(postOneDetail);
      path === "medicine" && setMedicineAll(res1);
      path === "news" && setPostDiff(postAll);
    };
  ...
  return (
    <div className="mt-[100px] flex gap-3">
      <div className="flex flex-col w-full justify-end min-lg:w-[70%] max-lg:p-5">
        <div className=" min-lg:w-[70%] max-lg:w-[100%] m-auto">
          <div className="flex flex-col gap-3">
            <Tag location={path === "medicine" ? medicineOne[0]?.nameMedicine : postDetail[0]?.nameMedicine} />
                ...
                return (
                  <div key={index} className="my-6 flex flex-col gap-3">
                    ...
                    <img
                      ...
                      alt="logo"
                    />
                    <div className="my-5 font-extralight">{item?.detail}</div>
      ...
      <div className="max-md:hidden w-[30%] pr-8">
        <h1 className="font-bold p-3 border-b-2 border-b-gray-800 ">
          Bài viết khác
        </h1>
        <ul className="mt-4">
          {pathname === "news" ? (
            postDiff.map((item, index) => {
              return (
            ...
            <div>
              {medicineAll.map((item, index) => {
                return (
                  <div key={index}>
                    <a key={index} href={`/medicine/${item?.id}`}>
                      <li key={index} className="flex my-2 gap-2">
                        <img
                          src={item?.imageMedicine}
                          className="w-[100px] h-[100px] rounded-md"
